...
public class Damier{
	...
	//dimensions de la zone d'affichage dans la fen�tre
	...
	//Fonction pour g�n�rer des nombres al�atoires ou non, en fonction de rigged
	private double rand(){
		if(rigged){
	...
	public Damier(int nbCasesHaut, int nbCasesLarg, int nbJetons, boolean rigged){
		...
		//Le -1 permet d'afficher la d�limitation du damier � droite et en bas, sinon le trac� sort de la zone d'affichage
	...
	//affiche le damier dans la console et dans une fen�tre graphique
	public void render(){
		/*affichage dans la console via tostring(), n'est plus n�c�ssaire puisqu'on se sert de l'API graphique*/
		...
		//Dessin des jetons
		/*� utiliser pour dessiner les cercles
		*/
		int r = Math.min(tileWidth,tileHeight)/2;
		
		/*� utiliser pour dessiner les formes incongrues
		...
		*/
		...
		//affichage � l'�cran des objets d�finis pr�c�dement
		window.flush();
		//attente d'atteindre environ le nombre d'images par secondes indiqu� par FPS
		window.wait((int)(1000/FPS));
	}
	
	//Donne une direction al�atoire � tous les jetons du damier
	...
	//Vrai si le jeton en param�tre va sortir du damier au prochain d�placement
	public boolean willMoveOut(Jeton j){
		return j.getNextX() < 0 || j.getNextX() > nbCasesLarg-1 || j.getNextY() < 0 || j.getNextY() > nbCasesHaut-1;
	}
	
	//Vrai si les jetons en param�tre vont entrer en collision au prochain d�placement
	...
	//Vrai si le jeton en param�tre a les m�me coordonn�es qu'un jeton du damier
	...
	//D�truit les jetons du damier n�cessaires afin qu'il n'y ait pas de collisions au prochain d�placement
	public void collide(){
		for(int i = 0; i <= this.tab.length-1; i++){
	...
	//Assigne une nouvelle direction valide aux jetons du damier qui devraient sortir du damier au prochain d�placement
	public void out(){
		for(int i = 0; i <= this.tab.length-1; i++){
	...
	//Renvoie le damier sous forme d'une chaine de caract�res
	...
	//Change la direction des jetons pour ne pas qu'il sortent du damier, puis suprimme les jetons requis pour �viter les collisions, puis d�place les jetons
	public void evolve(){
		this.out();
		this.collide();
		this.move();
